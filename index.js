...
function getUsersFromDatabase(array) {
  ...
  let mans = Array.from(array, (man) => ({
    id: man.id,
    firstName: man.name.toUpperCase(),
    years: man.age,
  }));
  return mans;
}
...
function removeLastElement(arr) {
  ...
  arr.length !== 0 ? arr.pop() : undefined;
  return arr;
}
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    ...
    let c = [];
    a !== b ? (c = c.concat([a], [b])) : (c = c.concat([a]));
    return c;
  } else {
    return [];
  }
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (arr.length >= target && start && end) {
...
function customAt(arr, index) {
  ...
  let elType = typeof arr.at(index);
  console.log(`Елемент є ${elType}.`);
  return arr.at(index);
}
...
function customIncludes(arr, element) {
  ...
  let f = arr.filter((item) => item === element).length;
  console.log(f);
